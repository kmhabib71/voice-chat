...
class ConversationMemory {
      ...
      if (stored) {
        ...
        if (parsed.keywords) {
          Object.keys(parsed.keywords).forEach(category => {
            if (parsed.keywords[category] && typeof parsed.keywords[category] === 'object') {
              parsed.keywords[category] = new Map(Object.entries(parsed.keywords[category]));
            }
          });
        }
    ...
    try {
      ...
      Object.keys(this.memory.keywords).forEach(category => {
        if (this.memory.keywords[category] instanceof Map) {
          memoryForStorage.keywords[category] = Object.fromEntries(this.memory.keywords[category]);
        }
      });
  ...
  updateKeywordFrequency(category, keyword, increment = 1) {
    ...
    const decayFactor = Math.pow(this.keywordDecayFactor, timeDiff / (24 * 60 * 60 * 1000)); // Daily decay
  ...
  async processMessage(text, isUser = true, emotion = 'neutral') {
    try {
      this.debugLog('Processing message', { text: text.substring(0, 50) + '...', isUser, emotion });
        ...
        if (Array.isArray(keywords[category])) {
          keywords[category].forEach(keyword => {
            if (keyword && keyword.trim()) {
              this.updateKeywordFrequency(category, keyword.toLowerCase().trim());
            }
          });
        }
      });
      ...
      if (this.memory.recentMessages.length > this.maxRecentMessages) {
        this.memory.recentMessages = this.memory.recentMessages.slice(-this.maxRecentMessages);
      }
      ...
      this.debugLog('Message processed', { 
        keywordsExtracted: Object.keys(keywords).reduce((acc, key) => acc + keywords[key].length, 0),
  ...
  inferConversationTone() {
    ...
    const topEmotion = Array.from(emotions.entries())
      .sort((a, b) => b[1].count - a[1].count)[0];
    const topIntent = Array.from(intents.entries())
      .sort((a, b) => b[1].count - a[1].count)[0];
  ...
  exportMemory() {
    ...
    Object.keys(this.memory.keywords).forEach(category => {
      if (this.memory.keywords[category] instanceof Map) {
        exported.keywords[category] = Object.fromEntries(this.memory.keywords[category]);
      }
    });
