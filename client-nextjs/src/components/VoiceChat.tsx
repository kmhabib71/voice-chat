...
function VoiceChat() {
      ...
      try {
        ...
        socket.on('disconnect', reason => {
          ...
          if (!isComponentMounted) {
            debugLog(
              'socketio',
              'Component unmounted, not attempting reconnect'
            );
            return;
          }
  ...
  useEffect(() => {
    ...
    debugLog(
      'continuous',
      `Continuous mode state changed to: ${isContinuousMode}`
    );
      ...
      if (audioRef.current) {
        ...
        const handlePlay = () => {
          ...
          if (!isContinuousModeRef.current && !analyserRef.current) {
            debugLog(
              'audio_interrupt',
              'Starting audio monitoring for interruption (not in continuous mode)'
            );
        ...
        if (playPromise !== undefined) {
          playPromise
            .then(() => {
              ...
              debugLog('audio', 'Audio playback started successfully', {
  ...
  const scheduleRecognitionRestart = (delay = 1000) => {
    ...
    if (recognitionRestartCountRef.current >= maxRestarts) {
      debugLog(
        'recovery',
        `Max restart attempts reached (${maxRestarts}), stopping continuous mode`
      );
      setStatus(
        '❌ Too many recognition errors - Please restart continuous mode manually'
      );
    ...
    const backoffDelay = Math.min(
      delay * Math.pow(1.5, recognitionRestartCountRef.current),
      10000
    );

    debugLog(
      'recovery',
      `Scheduling recognition restart in ${backoffDelay}ms (attempt ${recognitionRestartCountRef.current + 1})`
    );
    ...
    recognitionRestartTimeoutRef.current = setTimeout(() => {
      ...
      try {
        if (recognitionRef.current) {
          ...
          debugLog(
            'recovery',
            `Attempting to restart recognition (attempt ${recognitionRestartCountRef.current})`
          );
          ...
          setTimeout(() => {
            if (recognitionRestartCountRef.current > 0) {
              recognitionRestartCountRef.current = Math.max(
                0,
        ...
        if (
          ...
          recognitionRestartCountRef.current < maxRestarts
        ) {
  ...
  const setupContinuousRecognition = async () => {
    try {
      ...
      if ('grammars' in recognitionRef.current) {
        ...
        recognitionRef.current.grammars = new (window.SpeechGrammarList ||
      ...
      recognitionRef.current.onresult = event => {
        ...
        if (final) {
          ...
          debugLog('speech', 'Final transcript added', {
            final,
            total: currentTranscriptRef.current,
          });
        }
      ...
      recognitionRef.current.onerror = error => {
        ...
        switch (error.error) {
          case 'no-speech':
            debugLog(
              'recovery',
              'No speech detected - normal in continuous mode, will auto-restart'
            );
          ...
          case 'aborted':
            debugLog(
              'recovery',
              'Recognition aborted - likely due to mode change'
            );
          ...
          case 'audio-capture':
            debugLog(
              'recovery',
              'Audio capture error - checking microphone permissions'
            );
          ...
          case 'service-not-allowed':
            ...
            setStatus(
              '❌ Speech recognition service blocked - Check browser settings'
            );
            break;
          ...
          case 'language-not-supported':
            debugLog(
              'recovery',
              'Language/grammar error - using fallback settings'
            );
          ...
          default:
            debugLog(
              'recovery',
              `Unhandled error type: ${error.error} - will attempt restart`
            );
            if (isContinuousModeRef.current) {
      ...
      recognitionRef.current.onend = () => {
        ...
        if (isContinuousModeRef.current) {
          ...
          debugLog(
            'continuous',
            'Recognition ended - continuous mode disabled'
          );
        }
      };
  ...
  const monitorAudioLevels = () => {
    ...
    const checkAudioLevel = () => {
      ...
      if (!analyserRef.current || !isContinuousModeRef.current) {
        debugLog(
          'silence',
          `Audio monitoring stopped - continuous mode: ${isContinuousModeRef.current}, analyser: ${!!analyserRef.current}`
        );
        return;
      }
  ...
  const resetSilenceTimer = () => {
    ...
    if (!isContinuousModeRef.current) {
      debugLog(
        'silence',
        `Silence timer reset ignored - continuous mode: ${isContinuousModeRef.current}`
      );
      return;
    }
    ...
    silenceTimerRef.current = setTimeout(() => {
      ...
      if (!isContinuousModeRef.current) {
        debugLog(
          'silence',
          'Silence timer fired but continuous mode is inactive'
        );
        return;
      }
      ...
      if (currentTranscriptRef.current.trim() !== '') {
        debugLog(
          'silence',
          '1.5-second silence detected, processing transcript',
          {
  ...
  const processTranscriptSegment = async transcript => {
    try {
      ...
      if (!isContinuousModeRef.current) {
        debugLog(
          'segment',
          'Transcript processing cancelled - continuous mode inactive'
        );
        return;
      }
      ...
      debugLog(
        'user_message',
        'User message added from continuous mode',
        userMessage
      );
  ...
  const toggleContinuousMode = async () => {
    debugLog(
      'continuous',
      `Toggle continuous mode called - current state: ${isContinuousMode}`
    );
    ...
    if (isContinuousMode) {
      ...
      if (!socketRef.current || !socketRef.current.connected) {
        debugLog(
          'error',
          'Cannot start continuous mode - Socket.io not connected'
        );
      ...
      if (isAudioPlaying && audioRef.current) {
        debugLog(
          'speech_interrupt',
          'Stopping AI audio due to continuous mode activation'
        );
        audioRef.current.pause();
        setIsAudioPlaying(false);
      }
      ...
      setStatus(
        "Continuous mode active - Speak naturally, I'll respond after 3 seconds of silence"
      );
      currentTranscriptRef.current = '';
  ...
  return (
    <div className="container">
      ...
      <div className="status-indicator">
        ...
        {!isConnected && (
          <button onClick={reconnectSocket} className="retry-button">
            Retry
          </button>
        )}
        {isContinuousMode &&
          (lastRecognitionError || recognitionRestartCount > 3) && (
            <button
      ...
      <div className="chat-container" ref={chatContainerRef}>
        ...
        <div
          className={`typing-indicator ${isAIResponding ? 'visible' : 'hidden'}`}
        >
          🤖 AI is thinking...
        </div>
        ...
        {messages.length === 0 && !isAIResponding && (
          <div className="empty-state">
            Start a conversation by pressing the microphone button or typing
            below!
          </div>
        )}
      </div>
      ...
      <div className="debug-panel">
        ...
        <div>
          Last Speech: {Math.round((Date.now() - lastSpeechTime) / 1000)}s ago
        </div>
