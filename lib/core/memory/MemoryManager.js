...
class MemoryManager {
     ...
     */
    async searchEpisodicMemories(userId, query, limit = 5, keywordAnalysis = null) {
        try {
            this._validateUserId(userId);
            this._validateQuery(query);
            this._validateLimit(limit);

            const results = await this.episodicMemory.search(userId, query, limit, keywordAnalysis);
    ...
    /**
     * Store memory with intelligent context-aware routing
     * @param {string} userId - User identifier
     * @param {string} message - User message
     * @param {string} response - AI response
     * @param {Object} context - Conversation context (enhanced with extractedKeywords)
     * @param {string} memoryType - Type: 'short_term', 'long_term', 'episodic', 'auto'
     * @returns {Promise<Object>} Storage result with importance score and routing decision
     */
    async storeWithImportance(userId, message, response = '', context = {}, memoryType = 'auto') {
        try {
            ...
            // Calculate importance score with context awareness
            ...
            console.log(`📊 Importance Score: ${score} (Level: ${level})`);

            // Get context analysis for intelligent routing
            console.log('\n🧠 === CONTEXT ANALYSIS PHASE ===');
            let contextAnalysis = null;
            let routingDecision = null;

            if (context.extractedKeywords) {
                console.log('🔍 Running enhanced context analysis...');
                // Enhanced context analysis from AI keywords + local intelligence
                contextAnalysis = this.contextAnalyzer.analyzeContext(message, context.extractedKeywords, context);
                console.log('📊 Context Analysis Result:', JSON.stringify(contextAnalysis, null, 2));
                
                // Get intelligent routing recommendation
                routingDecision = this.contextAnalyzer.determineMemoryRouting(contextAnalysis, score);
                console.log('🎯 Routing Decision:', JSON.stringify(routingDecision, null, 2));
            } else {
                console.log('⚠️  No extracted keywords - skipping enhanced context analysis');
            }

            // Determine memory type with context-aware intelligence
            console.log('\n🎯 === MEMORY TYPE ROUTING PHASE ===');
            if (memoryType === 'auto') {
                if (routingDecision) {
                    // Use intelligent context-aware routing
                    memoryType = routingDecision.memoryType;
                    console.log(`✅ Context-aware routing: ${routingDecision.reasoning} → ${memoryType}`);
                } else {
                    // Fallback to importance-based routing
                    if (score >= 0.8) {
                    ...
                    console.log(`⚖️  Importance-based routing: score ${score} → ${memoryType}`);
                }
            } else {
                console.log(`🔒 Fixed memory type: ${memoryType}`);
            }

            // Enhanced context with importance data and context analysis
            ...
            console.log(`🎯 Final memory type: ${memoryType}`);
            let result;

            // Store based on determined type
            switch (memoryType) {
                case 'short_term':
                    console.log('📝 Storing in SHORT-TERM memory...');
                    ...
                    console.log('✅ Short-term storage complete:', result.success);
                    break;

                case 'long_term':
                    console.log('🧠 Storing in LONG-TERM memory...');
                    // Extract key-value for long term storage
                    const factData = this._extractFactFromMessage(message, enhancedContext);
                    if (factData) {
                        console.log('📊 Extracted fact data:', factData);
                        ...
                        console.log('✅ Long-term fact storage complete:', result.success);
                    } else {
                        console.log('⚠️  Fact extraction failed - falling back to short-term');
                        ...
                        console.log('✅ Fallback short-term storage complete:', result.success);
                    }
                    break;

                case 'episodic':
                    console.log('🎭 Storing in EPISODIC memory...');
                    
                    // Filter out low-value "I don't remember" conversations from episodic memory
                    if (response && (response.includes("I don't have specific memories") || 
                                   response.includes("I don't remember") ||
                                   response.includes("I don't recall"))) {
                        console.log('🚫 Skipping episodic storage: Low-value "I don\'t remember" response');
                        result = { 
                            success: true, 
                            skipped: true, 
                            reason: 'Filtered out low-value response',
                            memoryType: 'none' 
                        };
                        break;
                    }
                    ...
                    console.log('✅ Episodic storage complete:', result.success);
                    break;
            ...
            // Add importance and context information to result
            const finalResult = {
                ...result,
                importance: {
                    score,
                ...
                contextIntelligence: {
                    contextAnalysis: contextAnalysis,
                    routingDecision: routingDecision,
                    intelligentRouting: !!routingDecision,
                    routingReason: routingDecision ? routingDecision.reasoning : 'Importance-based routing'
                }
            };
            
            console.log('\n🎉 === MEMORY MANAGER: storeWithImportance COMPLETE ===');
            console.log('✅ Storage successful:', finalResult.success);
            console.log('📊 Final result:', {
                success: finalResult.success,
                memoryType: finalResult.importance.memoryType,
                importance: finalResult.importance.score,
                intelligentRouting: finalResult.contextIntelligence.intelligentRouting
            });
    ...
    async getEpisodicMemories(userId, limit = 10) {
        try {
            ...
            const episodes = await this.episodicMemory.getRecent(userId, limit);
            
            this._updateStats('episodic', 'retrieve');
            return episodes;

        } catch (error) {
            this._handleError('getEpisodicMemories', error);
            // Fallback to empty array for non-critical operation
            return [];
        }
    }
