...
/**
 * Memory importance scoring system with context awareness
...
class ImportanceScorer {
    ...
    /**
     * Calculate importance score for message content with context awareness
     * @param {string} message - User message
     * @param {string} response - AI response (optional)
     * @param {Object} context - Conversation context (enhanced with AI extraction)
    ...
    calculateImportance(message, response = '', context = {}) {
        try {
            ...
            // Get enhanced context analysis (if AI extraction available)
            const contextAnalysis = context.extractedKeywords ? 
                this.contextAnalyzer.analyzeContext(message, context.extractedKeywords, context) :
                null;

            // Context-aware importance adjustments
            if (contextAnalysis) {
                // Reduce importance for third-party information
                if (!contextAnalysis.information_ownership.about_user) {
                    score *= 0.6; // Significant reduction for others' information
                }

                // Reduce importance for hypothetical/speculative content
                if (contextAnalysis.temporal_context.timeframe === 'hypothetical' || 
                    contextAnalysis.certainty_analysis.level === 'speculative') {
                    score *= 0.4; // Major reduction for non-factual content
                }

                // Slight reduction for past events (unless very important)
                if (contextAnalysis.temporal_context.timeframe === 'past') {
                    score *= 0.85;
                }

                // Boost for high-confidence user information
                if (contextAnalysis.information_ownership.about_user && 
                    contextAnalysis.context_confidence > 0.8) {
                    score *= 1.2;
                }
            }

            // Enhanced pattern matching with context validation
            if (this.isEmotionalMilestone(messageContent, context, contextAnalysis)) {
                score += 0.5;
                highImportanceCount++;
            }
            if (this.isPersonalRevelation(messageContent, context, contextAnalysis)) {
                score += 0.4;
                highImportanceCount++;
            }
            if (this.isLifeEvent(messageContent, context, contextAnalysis)) {
                score += 0.4;
                highImportanceCount++;
            }

            // MEDIUM IMPORTANCE (+0.2 to +0.3) 
            if (this.isPreferenceDeclaration(messageContent, contextAnalysis)) {
                score += 0.35;  // Increased to ensure long-term storage (0.3 + 0.35 = 0.65 > 0.6)
                mediumImportanceCount++;
            }
            if (this.isGoalSetting(messageContent, contextAnalysis)) {
                score += 0.15;
                mediumImportanceCount++;
            }
    ...
    /**
     * Check if message contains emotional milestone indicators with context validation
     * @param {string} message - Message content
     * @param {Object} context - Conversation context
     * @param {Object} contextAnalysis - Enhanced context analysis
     * @returns {boolean}
     */
    isEmotionalMilestone(message, context = {}, contextAnalysis = null) {
        ...
        // Context validation: Only trigger if it's about the user
        if (contextAnalysis && !contextAnalysis.information_ownership.about_user) {
            return false; // Don't classify others' emotions as user's milestones
        }
    ...
    /**
     * Check if message contains personal revelation with context validation
     * @param {string} message - Message content
     * @param {Object} context - Conversation context
     * @param {Object} contextAnalysis - Enhanced context analysis
     * @returns {boolean}
     */
    isPersonalRevelation(message, context = {}, contextAnalysis = null) {
        ...
        // Context validation: Only trigger if it's about the user and factual
        if (contextAnalysis) {
            if (!contextAnalysis.information_ownership.about_user) {
                return false; // Don't classify others' revelations as user's
            }
            if (contextAnalysis.temporal_context.timeframe === 'hypothetical') {
                return false; // Don't classify hypothetical scenarios as revelations
            }
        }
    ...
    /**
     * Check if message contains life event indicators with context validation
     * @param {string} message - Message content
     * @param {Object} context - Conversation context
     * @param {Object} contextAnalysis - Enhanced context analysis
     * @returns {boolean}
     */
    isLifeEvent(message, context = {}, contextAnalysis = null) {
        ...
        // Context validation: Only trigger if it's about the user or user is directly involved
        if (contextAnalysis && 
            !contextAnalysis.information_ownership.about_user &&
            contextAnalysis.relationship_context.user_involvement !== 'participant') {
            return false; // Don't classify others' life events as user's unless user participated
        }
    ...
    /**
     * Check if message contains preference declaration with context validation
     * @param {string} message - Message content
     * @param {Object} contextAnalysis - Enhanced context analysis
     * @returns {boolean}
     */
    isPreferenceDeclaration(message, contextAnalysis = null) {
        // Context validation: Only trigger if it's about the user
        if (contextAnalysis && !contextAnalysis.information_ownership.about_user) {
            return false; // Don't classify others' preferences as user's
        }
    ...
    /**
     * Check if message contains goal setting with context validation
     * @param {string} message - Message content
     * @param {Object} contextAnalysis - Enhanced context analysis
     * @returns {boolean}
     */
    isGoalSetting(message, contextAnalysis = null) {
        // Context validation: Only trigger if it's about the user
        if (contextAnalysis && !contextAnalysis.information_ownership.about_user) {
            return false; // Don't classify others' goals as user's
        }
