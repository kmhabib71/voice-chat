    ...
    try {
      ...
      if (conversationMemory) {
        console.log('📊 Memory context structure:', {
          contextBuilt: conversationMemory.contextBuilt,
          userFactsCount: conversationMemory.userFacts?.length || 0,
          recentMemoriesCount: conversationMemory.recentMemories?.length || 0,
          episodicMemoriesCount: conversationMemory.episodicMemories?.length || 0
        });
        if (conversationMemory.episodicMemories?.length > 0) {
          console.log('🎭 Episodic memories content:');
          conversationMemory.episodicMemories.forEach((ep, i) => {
            console.log(`  Episodic ${i+1}: "${ep.summary?.substring(0, 80)}..."`);
          });
        }
      }
...
IMPORTANT: Use the context information below to provide accurate, personalized responses based on what you know about the user and their relationships.`;
      ...
      // Add Memory v2 context if available
      if (conversationMemory && conversationMemory.contextBuilt) {
        let contextInfo = '';
        
        // Add user facts context
        if (conversationMemory.userFacts && conversationMemory.userFacts.length > 0) {
          const recentFacts = conversationMemory.userFacts.slice(0, 3).map(f => f.value);
          contextInfo += ` User context: ${recentFacts.join(', ')}.`;
        }
        
        // Add relevant episodic memories context (from vector search)
        if (conversationMemory.episodicMemories && conversationMemory.episodicMemories.length > 0) {
          const episodicContext = conversationMemory.episodicMemories
            ...
            .filter(info => info && info.length > 10) // Only substantial information
            .join(', ');
        ...
        if (conversationMemory.recentMemories && conversationMemory.recentMemories.length > 0) {
          const recentContext = conversationMemory.recentMemories
            .slice(0, 2)
            .map(memory => memory.summary || 'Recent interaction')
            .join(', ');
          contextInfo += ` Recent: ${recentContext}.`;
        }
        
        if (contextInfo) {
          systemPrompt += ` Context: ${contextInfo}`;
        }
      }
